...
void RNN::setup_dropout_descriptor()
{
    ...
    cudnnErrCheck(cudnnSetDropoutDescriptor(m_dropoutDesc, m_Handle,
                                m_dropout, states, stateSize, seed));
}

void RNN::setup_rnn_descriptor()
{
    cudnnErrCheck(cudnnCreateRNNDescriptor(&m_rnnDesc));
    if      (m_mode == 0) m_RNNMode = CUDNN_RNN_RELU;
    else if (m_mode == 1) m_RNNMode = CUDNN_RNN_TANH;
    else if (m_mode == 2) m_RNNMode = CUDNN_LSTM;
    else if (m_mode == 3) m_RNNMode = CUDNN_GRU;

    
    if      (m_algo_int == 0) m_algo = CUDNN_RNN_ALGO_STANDARD;
    else if (m_algo_int == 1) m_algo = CUDNN_RNN_ALGO_PERSIST_STATIC;
    else if (m_algo_int == 2) m_algo = CUDNN_RNN_ALGO_PERSIST_DYNAMIC;

    //cudnnRNNAlgo_t algo = CUDNN_RNN_ALGO_STANDARD;
    //cudnnRNNAlgo_t algo = CUDNN_RNN_ALGO_PERSIST_STATIC;
    //cudnnRNNAlgo_t algo = CUDNN_RNN_ALGO_PERSIST_DYNAMIC;
    cudnnErrCheck(cudnnSetRNNDescriptor_v6(m_Handle, m_rnnDesc,
                                        m_hiddenSize, m_numLayers,
                                        m_dropoutDesc, CUDNN_LINEAR_INPUT, // We can also skip the input matrix transformation
                                        m_bidirectional ? CUDNN_BIDIRECTIONAL : CUDNN_UNIDIRECTIONAL,
                                        m_RNNMode, m_algo, //CUDNN_RNN_ALGO_STANDARD,
                                        CUDNN_DATA_FLOAT));
    // EDIT HERE
    if (m_algo_int == 2){
...
void RNN::setup_weights_and_inputs()
{
    ...
    if (m_hx != NULL) Wrapper::initGPUData((float*)m_hx, m_numLayers * m_hiddenSize * m_miniBatch * (m_bidirectional ? 2 : 1), 1.f);
    if (m_cx != NULL) Wrapper::initGPUData((float*)m_cx, m_numLayers * m_hiddenSize * m_miniBatch * (m_bidirectional ? 2 : 1), 1.f);

    Wrapper::initGPUData((float*)m_dy, m_seqLength * m_hiddenSize * m_miniBatch * (m_bidirectional ? 2 : 1), 1.f);
    if (m_dhy != NULL) Wrapper::initGPUData((float*)m_dhy, m_numLayers * m_hiddenSize * m_miniBatch * (m_bidirectional ? 2 : 1), 1.f);
    if (m_dcy != NULL) Wrapper::initGPUData((float*)m_dcy, m_numLayers * m_hiddenSize * m_miniBatch * (m_bidirectional ? 2 : 1), 1.f);
    ...
    for (int layer = 0; layer < m_numLayers * (m_bidirectional ? 2 : 1); layer++) {
        for (int linLayerID = 0; linLayerID < numLinearLayers; linLayerID++) {
            ...
            cudnnErrCheck(cudnnGetRNNLinLayerMatrixParams( m_Handle, m_rnnDesc, layer,
                                                            m_xDesc[0], m_wDesc, m_w,
                                                            linLayerID, linLayerMatDesc,
                                                            (void**)&linLayerMat));
            ...
            cudnnErrCheck(cudnnGetRNNLinLayerBiasParams( m_Handle, m_rnnDesc,
                                                         layer, m_xDesc[0],
                                                         m_wDesc, m_w,
                                                         linLayerID, linLayerBiasDesc,
                                                         (void**)&linLayerBias));
...
void RNN::train(){
    ...
    cudnnErrCheck(cudnnRNNForwardTraining(m_Handle,
                                            m_rnnDesc,
                                            m_seqLength,
                                            m_xDesc,
                                            m_x,
    ...
    cudnnErrCheck(cudnnRNNBackwardData(m_Handle,
                                m_rnnDesc,
                                m_seqLength,
                                m_yDesc,
                                m_y,
    ...
    cudnnErrCheck(cudnnRNNBackwardWeights( m_Handle,
                                        m_rnnDesc,
                                        m_seqLength,
                                        m_xDesc,
                                        m_x,
...
void RNN::print_checksums()
{
    ...
    if (true) {
        ...
        if (m_hy != NULL) cudaErrCheck(cudaMemcpy(testOutputh, m_hy, m_numLayers * m_hiddenSize * m_miniBatch * biDirScale * sizeof(float), cudaMemcpyDeviceToHost));
        if (m_cy != NULL && m_RNNMode == CUDNN_LSTM) cudaErrCheck(cudaMemcpy(testOutputc, m_cy, m_numLayers * m_hiddenSize * m_miniBatch * biDirScale * sizeof(float), cudaMemcpyDeviceToHost));
        ...
        for (int m = 0; m < m_miniBatch; m++) {
            ...
            for (int j = 0; j < m_numLayers * biDirScale; j++) {
                ...
                if (m_hy != NULL) localSumh += testOutputh[j * m_hiddenSize * m_miniBatch + m * m_hiddenSize + i];
                if (m_cy != NULL) if (m_RNNMode == CUDNN_LSTM) localSumc += testOutputc[j * m_hiddenSize * m_miniBatch + m * m_hiddenSize + i];
                }
            }
    ...
    if (true) {
        ...
        if (m_dhx != NULL) cudaErrCheck(cudaMemcpy(testOutputdh, m_dhx, m_numLayers * m_hiddenSize * m_miniBatch * biDirScale * sizeof(float), cudaMemcpyDeviceToHost));
        if (m_dcx != NULL) if (m_RNNMode == CUDNN_LSTM) cudaErrCheck(cudaMemcpy(testOutputdc, m_dcx, m_numLayers * m_hiddenSize * m_miniBatch * biDirScale * sizeof(float), cudaMemcpyDeviceToHost));
        ...
        for (int m = 0; m < m_miniBatch; m++) {
            ...
            for (int j = 0; j < m_numLayers * biDirScale; j++) {
                ...
                if (m_RNNMode == CUDNN_LSTM) localSumdc += testOutputdc[j * m_hiddenSize * m_miniBatch + m * m_hiddenSize + i];
                }
            }
...
bool RNN::read_input_file()
{
    ...
    io::CSVReader<19> input_stream(m_input_file);
    input_stream.read_header(io::ignore_missing_column | io::ignore_extra_column, "engine_id", "cycle", "setting1", "setting2", "temp_lpc_outlet", "temp_hpc_outlet", "temp_lpt_outlet", "pressure_hpc_outlet", "physical_fan_speed", "physical_core_speed", "static_pressure_hpc_outlet", "fuel_flow_ration_ps30", "corrected_fan_speed", "corrected_core_speed", "bypass_ratio", "bleed_enthalpy", "hpt_collant_bleed", "lpt_coolant_bleed", "ttf");

    try
    {
        float engine_id, cycle, setting1, setting2, temp_lpc_outlet, temp_hpc_outlet, temp_lpt_outlet, pressure_hpc_outlet, physical_fan_speed, physical_core_speed, static_pressure_hpc_outlet, fuel_flow_ration_ps30, corrected_fan_speed, corrected_core_speed, bypass_ratio, bleed_enthalpy, hpt_collant_bleed, lpt_coolant_bleed, ttf;

        while (input_stream.read_row(engine_id, cycle, setting1, setting2, temp_lpc_outlet, temp_hpc_outlet, temp_lpt_outlet, pressure_hpc_outlet, physical_fan_speed, physical_core_speed, static_pressure_hpc_outlet, fuel_flow_ration_ps30, corrected_fan_speed, corrected_core_speed, bypass_ratio, bleed_enthalpy, hpt_collant_bleed, lpt_coolant_bleed, ttf))
        {
            std::unique_ptr<measurement> new_measurement = std::make_unique<measurement>(engine_id, cycle, setting1, setting2, temp_lpc_outlet, temp_hpc_outlet, temp_lpt_outlet, pressure_hpc_outlet, physical_fan_speed, physical_core_speed, static_pressure_hpc_outlet, fuel_flow_ration_ps30, corrected_fan_speed, corrected_core_speed, bypass_ratio, bleed_enthalpy, hpt_collant_bleed, lpt_coolant_bleed, ttf);

            m_measurements.push_back(std::move(new_measurement));
        }
    }
    ...
    spdlog::info("Loaded {} measurement from {}", m_measurements.size(), file_name());
    return true;
}
